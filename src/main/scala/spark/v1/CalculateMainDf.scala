...
object CalculateMainDf extends App {
  ...
  val dir = new File(ProcessConstants.DATA_FOLDER + "weighted.news/")
  ...
  val writeDir = new File(ProcessConstants.DATA_FOLDER + "mainDf/")
  FileUtils.deleteRecursively(writeDir)

  // We format the dataframe as a CSV file and save it as a text file
  df.rdd.map(_.mkString(",").replaceAll("null", "")).saveAsTextFile("src/main/data/mainDf")
}
